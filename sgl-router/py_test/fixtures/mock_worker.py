...
class SimpleMockTokenizer:  
    ...
    def decode(self, token_ids: list[int]) -> str:  
        ...
        return " ".join(tokens)  # 用空格连接,匹配Rust实现
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--worker-type",   
                   ...
                   default="regular",  
                   help="Worker type for PD disaggregation")  
    p.add_argument("--bootstrap-port", type=int, default=None,  
                   help="Bootstrap port for prefill workers")
...
def create_app(args: argparse.Namespace) -> FastAPI:
    ...
    async def get_server_info(request: Request):  
        ...
        # 构建worker类型信息  
        worker_type_info = {"type": args.worker_type}  
        if args.worker_type == "prefill" and args.bootstrap_port:  
            worker_type_info["bootstrap_port"] = args.bootstrap_port  
        ...
        return JSONResponse(  
            {  
                ...
                "worker_type": worker_type_info,  # 添加worker类型信息  
            }  
        )
    ...
    @app.get("/v1/radixtree/full")  
    async def get_radixtree():  
        tokenizer = SimpleMockTokenizer()  
        
        # 使用真实的prompt进行tokenization  
        ...
        return JSONResponse({  
            ...
            "tree": {  
                "key": tokens[:5],  # 前5个token作为前缀  
                "children": [  
                    {  
                        "key": tokens[5:],  # 剩余token作为子节点  
                        "children": []  
                    }  
                ]  
            }  
