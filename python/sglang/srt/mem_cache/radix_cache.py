...
class RadixCache(BasePrefixCache):
    ...
    def lmcache_connector_enabled(self):
        """
        判断当前对象是否启用了LMCache连接器。

        Args:
        无。

        Returns:
        bool: 如果当前对象具有LMCache连接器且不为None,则返回True;否则返回False。
    ...
    def match_prefix(self, key: RadixKey, **kwargs) -> MatchResult:
        ...
        * Separate requests that intentionally should not share state (e.g.,
        ...
        if self.lmcache_connector_enabled():
            ...
            if isinstance(key, RadixKey):  
                key_tokens = torch.tensor(key.token_ids, device=self.device)  
            else:  
            ...
            if num_retrieved_tokens > 0:
                ...
                last_node.children[self.get_child_key_fn(key[len(value):len(value) 
                                                             + (num_retrieved_tokens - prefix_padding_len)])] = new_node
                last_node = new_node
